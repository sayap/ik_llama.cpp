...
// IQK (QK_K=256) and KT quant families. These only get mul_mat_vec and
// dequant shaders: the mat-mat path uses the dequant-to-F16 fallback.
...
void matmul_shaders(bool fp16, bool matmul_id, bool coopmat, bool coopmat2, bool f16acc) {
    ...
    // bf16
    {
        ...
        // base_dict always maps FLOAT_TYPE_VEC2 to f16vec2 for fp16 builds, but
        // the bf16 shaders use bfloat16 (or float for the scalar path), so the
        // vec2 type must be overridden here.
        const std::string float_type_vec2 = (coopmat || coopmat2) ? "bf16vec2" : "vec2";
...
#endif
        {
            string_to_spv(shader_name + "_bf16_aligned", source_name, merge_maps(base_dict, {{"FLOAT_TYPE", FLOAT_TYPE("bf16")}, {"FLOAT_TYPE_VEC2", float_type_vec2}, {"TO_FLOAT_TYPE", to_float_type}, {"DATA_A_BF16", "1"}, {"LOAD_VEC_A", load_vec_a},           {"LOAD_VEC_B", "4"}, {"B_TYPE", coopmat2 ? "bfloat16_t" : "u16vec4"},   {"D_TYPE", "float"}, {"B_IS_FLOAT", "1"}, {"ALIGNED", "1"}}), fp16, coopmat, coopmat2, f16acc);
            string_to_spv(shader_name + "_bf16",         source_name, merge_maps(base_dict, {{"FLOAT_TYPE", FLOAT_TYPE("bf16")}, {"FLOAT_TYPE_VEC2", float_type_vec2}, {"TO_FLOAT_TYPE", to_float_type}, {"DATA_A_BF16", "1"}, {"LOAD_VEC_A", load_vec_a_unaligned},                      {"B_TYPE", coopmat2 ? "bfloat16_t" : "uint16_t"},                          {"D_TYPE", "float"}, {"B_IS_FLOAT", "1"}}),                   fp16, coopmat, coopmat2, f16acc);
        }
    }
    ...
    for (const auto& tname : type_names) {
        ...
        if ((tname == "q4_0") || (tname == "q4_1") || (tname == "iq1_s") || (tname == "iq1_m") || (tname == "iq2_xxs") || (tname == "iq2_xs") || (tname == "iq2_s"))
            load_vec_quant = "8";
        else if ((tname == "q2_k") || (tname == "q4_k") || (tname == "q5_k") || (tname == "q5_0") || (tname == "q5_1") || (tname == "q6_0") || (tname == "q8_0") || (tname == "iq3_xxs") || (tname == "iq3_s") || (tname == "iq4_nl") || (tname == "mxfp4"))
            load_vec_quant = "4";
        ...
        if (!coopmat && !coopmat2 && !matmul_id && (tname == "q4_0" || tname == "q4_1" || tname == "q5_0" || tname == "q5_1" || tname == "q8_0" || tname == "q2_k" || tname == "q3_k" || tname == "q4_k" || tname == "q5_k" || tname == "q6_k")) {
    ...
    if (!coopmat2 && !matmul_id) {
        ...
        for (size_t ti = 0; ti < iqk_type_names.size(); ++ti) {
            const std::string& tname = iqk_type_names[ti];
            std::string data_a_key = "DATA_A_" + to_uppercase(tname);
    ...
    if (coopmat2) {
        ...
        // address from the push constants; B is F16 (the activations are
        ...
        iqk_cm2_defines["GGML_VULKAN_COOPMAT2_DECODE_VECTOR"] = "1";
#endif
        for (const auto& tname : iqk_type_names) {
            ...
            string_to_spv(shader_name + "_" + tname + "_f16", source_name, merge_maps(base_dict, merge_maps(iqk_cm2_defines, {{data_a_key, "1"}, {"LOAD_VEC_A", "1"}, {"LOAD_VEC_B", "1"}})), fp16, coopmat, coopmat2, f16acc);
            string_to_spv(shader_name + "_" + tname + "_f16_aligned", source_name, merge_maps(base_dict, merge_maps(iqk_cm2_defines, {{data_a_key, "1"}, {"LOAD_VEC_A", load_vec}, {"LOAD_VEC_B", load_vec}, {"B_TYPE", aligned_b_type_f16}, {"ALIGNED", "1"}})), fp16, coopmat, coopmat2, f16acc);
        }
    }
}
...
void process_shaders() {
    ...
    for (const auto& f16acc : {false, true}) {
        ...
        for (const auto& tname : type_names) {
            ...
            } else if (tname == "q4_0" || tname == "q6_0" || tname == "q8_0") {
                std::string data_a_key = "DATA_A_" + to_uppercase(tname);
                string_to_spv("flash_attn_f32_f16_" + tname, "flash_attn_cm1.comp",
    ...
    for (const auto& tname : std::vector<std::string>{ "iq1_kt", "iq2_kt", "iq3_kt", "iq4_kt", "iq2_ks", "iq2_k", "iq2_kl", "iq3_ks", "iq3_k", "iq4_kss", "iq4_ks", "iq4_k", "iq5_ks", "iq5_k", "iq6_k", "q6_0" }) {
        const std::string data_a_key = "DATA_A_" + to_uppercase(tname);
    ...
    for (std::string t : {"q4_0", "q4_1", "q5_0", "q5_1", "q6_0", "q8_0", "iq4_nl"}) {
    ...
    for (std::string t : {"f32", "f16", "bf16", "q4_0", "q4_1", "q5_0", "q5_1", "q6_0", "q8_0", "iq4_nl"}) {
