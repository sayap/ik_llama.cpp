...
#define LOAD_CUDA_FN(name) g_cuda.name = (decltype(g_cuda.name)) dlsym(g_cuda.lib, #name)
    ...
    LOAD_CUDA_FN(cuMemcpyHtoD_v2);
    LOAD_CUDA_FN(cuMemcpyDtoH_v2);
...
#undef LOAD_CUDA_FN
    ...
    if (!g_cuda.cuInit || !g_cuda.cuDeviceGet || !g_cuda.cuDevicePrimaryCtxRetain ||
        ...
        !g_cuda.cuMemcpyHtoD_v2 || !g_cuda.cuMemcpyDtoH_v2) {
        dlclose(g_cuda.lib);
        g_cuda = vk_cuda_api{};
        return;
    }
...
struct vk_device_struct {
    ...
    // Availability of the q8_1 (integer-dot) mmq tiles. These have their own
    // (larger, BK_STEP=4) shared-memory layout, so they are gated separately
    // from the f16-dequant mul_mat_* flags (see ggml_vk_matmul_int_shmem_support).
    ...
    vk_pipeline pipeline_set_rows[2][GGML_TYPE_COUNT];
    ...
    vk_pipeline pipeline_mask_to_idx[2];
    vk_pipeline pipeline_indexer_topk_score[2][2];   // [k f32/f16][mask f32/f16]
    vk_pipeline pipeline_indexer_topk_select;
    vk_pipeline pipeline_indexer_topk_select_bitonic;   // shared-memory bitonic path, big-k fast path
    vk_pipeline pipeline_latent_attn[3];
    vk_pipeline pipeline_flash_attn_indexed[2];
    vk_pipeline pipeline_mul_multi_add_f32;
    // [log2(nh) - 6][0=f32, 1=f16] for nh in {64,128,256,512}
    vk_pipeline pipeline_hadamard[4][2];
    // [0=f32, 1=f16]
    vk_pipeline pipeline_fill[2];
    vk_pipeline pipeline_get_rows_dim0[2];   // [0=f32, 1=f16]
    ...
    vk_pipeline pipeline_sqrt_softplus[2];
...
struct vk_op_sinkhorn_push_constants {
    uint32_t T;
    uint32_t S;
    uint32_t iters;
    float eps;
...
struct vk_op_hc_pre_push_constants {
    uint32_t T;
    uint32_t S;
    uint32_t iters;
    float eps;
...
struct vk_op_hc_post_push_constants {
    uint32_t ne0;
    uint32_t T;
    uint32_t S;
    uint32_t x_nb1;
...
struct vk_op_ds4_comp_push_constants {
    uint32_t type;
    uint32_t ne0;
    uint32_t nblock;
    uint32_t ratio;
...
struct vk_op_mask_topk_push_constants {
    uint32_t ne0;
    uint32_t ntopk;
    uint32_t topk_ne1;
    uint32_t ne1;
...
struct vk_op_mask_to_idx_push_constants {
    uint32_t ne00;
    uint32_t ne0;
    uint32_t ne1;
    uint32_t ne2;
...
struct vk_op_indexer_topk_score_push_constants {
    uint32_t n_kv;
    uint32_t Dk;
    uint32_t n_head;
    uint32_t tile_rows;
...
struct vk_op_indexer_topk_select_push_constants {
    uint32_t n_kv;
    uint32_t n_top_k;
    uint32_t first_row;
    uint32_t dst_nb1;
...
struct vk_op_hadamard_push_constants {
    uint32_t nc;
    uint32_t src_nb1;
    uint32_t src_nb2;
    uint32_t src_nb3;
...
struct vk_op_get_rows_dim0_push_constants {
    uint32_t ne0;
    uint32_t ne1;
    uint32_t ne2;
    uint32_t ne3;
...
struct vk_op_latent_attn_push_constants {
    uint32_t Dk;
    uint32_t T;
    uint32_t H;
    uint32_t N;
...
// Strip the SPV_NV_cooperative_matrix_decode_vector extension (and the
// DecodeVectorFunc tensor-addressing bit) from a SPIR-V module, so a shader
...
static bool ggml_vk_strip_decode_vector(const uint32_t * code, size_t word_count, std::vector<uint32_t> & out) {
    ...
    for (size_t pos = 5; pos < word_count; ) {
        ...
        if (op == spv::OpCapability && wc == 2 && code[pos + 1] == spv::CapabilityCooperativeMatrixDecodeVectorNV) {
            flush_run(pos);
            pos += wc;
            run_start = pos;
            continue;
        ...
        if (op == spv::OpCooperativeMatrixLoadTensorNV) {
            ...
            if ((ta_mask & spv::TensorAddressingOperandsDecodeVectorFuncMask) == 0) {
                pos += wc;
            ...
            out.push_back(ta_mask & ~spv::TensorAddressingOperandsDecodeVectorFuncMask);
...
static void ggml_vk_create_pipeline_func(vk_device& device, vk_pipeline& pipeline, size_t spv_size, const void* spv_data, const std::string entrypoint,
    ...
    if (device->coopmat2 && !device->coopmat2_decode_vector) {
        if (ggml_vk_strip_decode_vector(reinterpret_cast<const uint32_t *>(spv_data), spv_size / sizeof(uint32_t), stripped)) {
            shader_module_create_info = vk::ShaderModuleCreateInfo({}, stripped.size() * sizeof(uint32_t), stripped.data());
        }
    }
#endif
...
// Shmem usage for the q8_1 mmq shader (mul_mmq.comp), which stages BK_STEP=4
// K-tiles through block_a_cache / block_b_cache (see mul_mmq.comp) instead of
// the float load buffers checked by ggml_vk_matmul_shmem_support. Sizes follow
...
static bool ggml_vk_matmul_int_shmem_support(const vk_device& device, const std::vector<uint32_t>& warptile, bool mul_mat_id, ggml_type src0_type) {
    ...
    auto std430_size = [](std::initializer_list<member> members) {
        ...
        for (const auto &m : members) {
            ...
            off = (off + m.align - 1) / m.align * m.align + m.size;
        }
        return (off + struct_align - 1) / struct_align * struct_align;
    };

    // block_a_cache per type (mul_mmq.comp). The generic staging types
    // (per-32-scale IQK/KT, Q6_0, MXFP4, Q4_0/Q5_0/Q8_0) carry a single
    // FLOAT_TYPE scale; the byte-packed types carry a FLOAT_TYPE_VEC2 pair.
    uint32_t block_a_size;
    switch (src0_type) {
    case GGML_TYPE_Q2_K:
        // qs[2] + u8vec2 scales + FLOAT_TYPE_VEC2 dm
        block_a_size = std430_size({{8, 4}, {2, 2}, {fp2_size, fp2_align}});
        break;
    case GGML_TYPE_Q3_K:
    case GGML_TYPE_Q4_K:
        // qs[4] + FLOAT_TYPE_VEC2 dm
...
static void ggml_vk_load_shaders(vk_device& device) {
    ...
    if (device->coopmat2) {
        ...
        if ((device->architecture == AMD_GCN) && (device->driver_id != vk::DriverId::eAmdProprietary)) {
            ...
            l_warptile_mmq = { 256, 128, 128, 32, subgroup_size_8, 64, 2, tm_m, tn_m, tk_m, subgroup_size_8 };
            // The scalar int mmq cannot use the coopmat TM/TN (WNITER would
            // collapse to 0 in mul_mmq.comp); mirror the K-quant tile shape
            // instead: BLOCK_SIZE=256 (4 warps) over the same 128x128 tile
            // halves the per-thread accumulator count (128 -> 64 f32) vs the
            // generic tile, which was register-bound on RDNA.
            l_warptile_mmq_int   = { 256, 128, 128, 32, subgroup_size_16, 64, 2, 4, 2, 1, subgroup_size_16 };
        ...
        for (uint32_t i = 0; i < GGML_TYPE_COUNT; ++i) {
            ...
            // The q8_1 mmq path has its own (larger, BK_STEP=4) shared memory
            // layout, check it separately. K-quants use the _int_k warptiles,
            // the per-32-scale types use _int.
...
#define CREATE_FA(TYPE, NAMELC, FAPATH, SUFFIX) \
    ...
    CREATE_FA(GGML_TYPE_Q6_0, q6_0, FA_SCALAR, )
    CREATE_FA(GGML_TYPE_Q8_0, q8_0, FA_SCALAR, )
#if defined(VK_KHR_cooperative_matrix) && defined(GGML_VULKAN_COOPMAT_GLSLC_SUPPORT)
    if (device->coopmat1_fa_support) {
        CREATE_FA(GGML_TYPE_F16, f16, FA_COOPMAT1, _cm1)
        CREATE_FA(GGML_TYPE_Q4_0, q4_0, FA_COOPMAT1, _cm1)
        CREATE_FA(GGML_TYPE_Q6_0, q6_0, FA_COOPMAT1, _cm1)
    ...
    if (device->coopmat2) {
        ...
        CREATE_FA(GGML_TYPE_Q6_0, q6_0, FA_COOPMAT2, _cm2)
...
#undef CREATE_MM2_L
        ...
        CREATE_MM(pipeline_dequant_mul_mat_mat_id[GGML_TYPE_Q6_0].f16acc, matmul_id_q6_0_f16, , mmqid_wg_denoms, warptile_mmqid, vk_mat_mat_id_push_constants, 4)
...
#define CREATE_MM2(TYPE, PIPELINE_NAME, NAMELC, WG_DENOMS, WARPTILE, PUSHCONST, PARAMCOUNT, ID) \
        ...
        if (device->coopmat_acc_f16_support) {
            ...
            // IQK/KT inline-dequant A-tiles (byte-addressed, see mul_mm.comp)
            CREATE_MM2(GGML_TYPE_IQ2_K,   pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ2_K],   matmul_iq2_k_f32,   mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM2(GGML_TYPE_IQ3_K,   pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ3_K],   matmul_iq3_k_f32,   mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            ...
            CREATE_MM2(GGML_TYPE_IQ2_KS,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ2_KS],  matmul_iq2_ks_f32,  mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM2(GGML_TYPE_IQ3_KS,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ3_KS],  matmul_iq3_ks_f32,  mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            ...
            CREATE_MM2(GGML_TYPE_IQ4_KSS, pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ4_KSS], matmul_iq4_kss_f32, mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM2(GGML_TYPE_IQ5_KS,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ5_KS],  matmul_iq5_ks_f32,  mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM2(GGML_TYPE_IQ2_KL,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ2_KL],  matmul_iq2_kl_f32,  mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM2(GGML_TYPE_IQ1_KT,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ1_KT],  matmul_iq1_kt_f32,  mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM2(GGML_TYPE_IQ2_KT,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ2_KT],  matmul_iq2_kt_f32,  mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM2(GGML_TYPE_IQ3_KT,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ3_KT],  matmul_iq3_kt_f32,  mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            ...
            CREATE_MM(GGML_TYPE_IQ2_K,   pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ2_K].f32acc,   matmul_iq2_k_f32,   , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM(GGML_TYPE_IQ3_K,   pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ3_K].f32acc,   matmul_iq3_k_f32,   , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            ...
            CREATE_MM(GGML_TYPE_IQ2_KS,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ2_KS].f32acc,  matmul_iq2_ks_f32,  , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM(GGML_TYPE_IQ3_KS,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ3_KS].f32acc,  matmul_iq3_ks_f32,  , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            ...
            CREATE_MM(GGML_TYPE_IQ4_KSS, pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ4_KSS].f32acc, matmul_iq4_kss_f32, , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM(GGML_TYPE_IQ5_KS,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ5_KS].f32acc,  matmul_iq5_ks_f32,  , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM(GGML_TYPE_IQ2_KL,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ2_KL].f32acc,  matmul_iq2_kl_f32,  , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM(GGML_TYPE_IQ1_KT,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ1_KT].f32acc,  matmul_iq1_kt_f32,  , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM(GGML_TYPE_IQ2_KT,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ2_KT].f32acc,  matmul_iq2_kt_f32,  , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM(GGML_TYPE_IQ3_KT,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ3_KT].f32acc,  matmul_iq3_kt_f32,  , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
...
#undef CREATE_MM
    ...
    for (uint32_t i = 0; i < mul_mat_vec_max_cols; ++i) {
        ...
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_Q6_0][i], "mul_mat_vec_q6_0_f32_f32_"+std::to_string(i+1), mul_mat_vec_q6_0_f32_f32_len, mul_mat_vec_q6_0_f32_f32_data, "main", 3, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, i+1}, 1, true);
        ...
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[GGML_TYPE_MXFP4][i], "mul_mat_vec_mxfp4_f32_f32_"+std::to_string(i+1), mul_mat_vec_mxfp4_f32_f32_len, mul_mat_vec_mxfp4_f32_f32_data, "main", 3, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, i+1}, 1, true);
        ...
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_Q6_0][i], "mul_mat_vec_q6_0_f16_f32_"+std::to_string(i+1), mul_mat_vec_q6_0_f16_f32_len, mul_mat_vec_q6_0_f16_f32_data, "main", 3, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, i+1}, 1, true);
        ...
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[GGML_TYPE_MXFP4][i], "mul_mat_vec_mxfp4_f16_f32_"+std::to_string(i+1), mul_mat_vec_mxfp4_f16_f32_len, mul_mat_vec_mxfp4_f16_f32_data, "main", 3, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq, i+1}, 1, true);
...
#define CREATE_MMV_IQK(TYPE, NAMELC) \
        ...
        CREATE_MMV_IQK(GGML_TYPE_IQ5_K, iq5_k)
        CREATE_MMV_IQK(GGML_TYPE_IQ6_K, iq6_k)
        CREATE_MMV_IQK(GGML_TYPE_IQ2_KS, iq2_ks)
        CREATE_MMV_IQK(GGML_TYPE_IQ3_KS, iq3_ks)
        CREATE_MMV_IQK(GGML_TYPE_IQ4_KS, iq4_ks)
        CREATE_MMV_IQK(GGML_TYPE_IQ4_KSS, iq4_kss)
        CREATE_MMV_IQK(GGML_TYPE_IQ5_KS, iq5_ks)
        CREATE_MMV_IQK(GGML_TYPE_IQ2_KL, iq2_kl)
        CREATE_MMV_IQK(GGML_TYPE_IQ1_KT, iq1_kt)
        CREATE_MMV_IQK(GGML_TYPE_IQ2_KT, iq2_kt)
        CREATE_MMV_IQK(GGML_TYPE_IQ3_KT, iq3_kt)
...
#undef CREATE_MMV_Q8_1
    ...
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_Q6_0], "mul_mat_vec_id_q6_0_f32", mul_mat_vec_id_q6_0_f32_len, mul_mat_vec_id_q6_0_f32_data, "main", 4, sizeof(vk_mat_vec_id_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq}, 1, true);
    ...
    ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[GGML_TYPE_MXFP4], "mul_mat_vec_id_mxfp4_f32", mul_mat_vec_id_mxfp4_f32_len, mul_mat_vec_id_mxfp4_f32_data, "main", 4, sizeof(vk_mat_vec_id_push_constants), {2*rm_stdq, 1, 1}, {device->subgroup_size, 2*rm_stdq}, 1, true);
...
#define CREATE_MMV_ID_IQK(TYPE, NAMELC) \
    ...
    CREATE_MMV_ID_IQK(GGML_TYPE_IQ5_K, iq5_k)
    CREATE_MMV_ID_IQK(GGML_TYPE_IQ6_K, iq6_k)
    CREATE_MMV_ID_IQK(GGML_TYPE_IQ2_KS, iq2_ks)
    CREATE_MMV_ID_IQK(GGML_TYPE_IQ3_KS, iq3_ks)
    CREATE_MMV_ID_IQK(GGML_TYPE_IQ4_KS, iq4_ks)
    CREATE_MMV_ID_IQK(GGML_TYPE_IQ4_KSS, iq4_kss)
    CREATE_MMV_ID_IQK(GGML_TYPE_IQ5_KS, iq5_ks)
    CREATE_MMV_ID_IQK(GGML_TYPE_IQ2_KL, iq2_kl)
    CREATE_MMV_ID_IQK(GGML_TYPE_IQ1_KT, iq1_kt)
    CREATE_MMV_ID_IQK(GGML_TYPE_IQ2_KT, iq2_kt)
    CREATE_MMV_ID_IQK(GGML_TYPE_IQ3_KT, iq3_kt)
    CREATE_MMV_ID_IQK(GGML_TYPE_IQ4_KT, iq4_kt)
#undef CREATE_MMV_ID_IQK
...
#define CREATE_DEQUANT_IQK(TYPE, NAMELC, DENOM) \
    ...
    CREATE_DEQUANT_IQK(GGML_TYPE_IQ5_K,  iq5_k,  256 * 16)
    CREATE_DEQUANT_IQK(GGML_TYPE_IQ6_K,  iq6_k,  256 * 16)
    ...
    CREATE_DEQUANT_IQK(GGML_TYPE_IQ5_KS, iq5_ks, 256 * 16)
...
#undef CREATE_DEQUANT_IQK
    ...
    ggml_vk_create_pipeline(device, device->pipeline_quantize_q8_1, "quantize_q8_1", quantize_q8_1_len, quantize_q8_1_data, "main", 2, 2 * sizeof(uint32_t), {32 * device->subgroup_size / 8, 1, 1}, { device->subgroup_size }, 1);
    if (device->subgroup_add && device->subgroup_require_full_support) {
        ggml_vk_create_pipeline(device, device->pipeline_quantize_q8_1_x4, "quantize_q8_1_x4", quantize_q8_1_x4_subgroup_len, quantize_q8_1_x4_subgroup_data, "main", 2, 2 * sizeof(uint32_t), {32 * device->subgroup_size / 8, 1, 1}, { device->subgroup_size }, 1, true, true);
    } else {
        ggml_vk_create_pipeline(device, device->pipeline_quantize_q8_1_x4, "quantize_q8_1_x4", quantize_q8_1_x4_len, quantize_q8_1_x4_data, "main", 2, 2 * sizeof(uint32_t), {32 * device->subgroup_size / 8, 1, 1}, { device->subgroup_size }, 1);
    }
    ...
    ggml_vk_create_pipeline(device, device->pipeline_mask_topk[1], "mask_topk_f16", mask_topk_f16_len, mask_topk_f16_data, "main", 3, sizeof(vk_op_mask_topk_push_constants), {256, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_mask_to_idx[0], "mask_to_idx_f32", mask_to_idx_f32_len, mask_to_idx_f32_data, "main", 2, sizeof(vk_op_mask_to_idx_push_constants), {256, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_mask_to_idx[1], "mask_to_idx_f16", mask_to_idx_f16_len, mask_to_idx_f16_data, "main", 2, sizeof(vk_op_mask_to_idx_push_constants), {256, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_indexer_topk_score[0][0], "indexer_topk_score_f32", indexer_topk_score_f32_len, indexer_topk_score_f32_data, "main", 5, sizeof(vk_op_indexer_topk_score_push_constants), {256, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_indexer_topk_score[0][1], "indexer_topk_score_f32_m16", indexer_topk_score_f32_m16_len, indexer_topk_score_f32_m16_data, "main", 5, sizeof(vk_op_indexer_topk_score_push_constants), {256, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_indexer_topk_score[1][0], "indexer_topk_score_f16", indexer_topk_score_f16_len, indexer_topk_score_f16_data, "main", 5, sizeof(vk_op_indexer_topk_score_push_constants), {256, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_indexer_topk_score[1][1], "indexer_topk_score_f16_m16", indexer_topk_score_f16_m16_len, indexer_topk_score_f16_m16_data, "main", 5, sizeof(vk_op_indexer_topk_score_push_constants), {256, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_indexer_topk_select, "indexer_topk_select_f32", indexer_topk_select_f32_len, indexer_topk_select_f32_data, "main", 2, sizeof(vk_op_indexer_topk_select_push_constants), {256, 1, 1}, {}, 1);
    if (device->properties.limits.maxComputeSharedMemorySize >= 36u*1024u) {
        ggml_vk_create_pipeline(device, device->pipeline_indexer_topk_select_bitonic, "indexer_topk_select_bitonic_f32", indexer_topk_select_bitonic_f32_len, indexer_topk_select_bitonic_f32_data, "main", 2, sizeof(vk_op_indexer_topk_select_push_constants), {256, 1, 1}, {}, 1);
    }
    ...
    ggml_vk_create_pipeline(device, device->pipeline_flash_attn_indexed[0], "flash_attn_indexed_f16", flash_attn_indexed_f16_len, flash_attn_indexed_f16_data, "main", 7, sizeof(vk_op_flash_attn_indexed_push_constants), {128, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_flash_attn_indexed[1], "flash_attn_indexed_q8_0", flash_attn_indexed_q8_0_len, flash_attn_indexed_q8_0_data, "main", 7, sizeof(vk_op_flash_attn_indexed_push_constants), {128, 1, 1}, {}, 1);

    ggml_vk_create_pipeline(device, device->pipeline_mul_multi_add_f32, "mul_multi_add_f32", mul_multi_add_f32_len, mul_multi_add_f32_data, "main", 5, sizeof(vk_op_mul_multi_add_push_constants), {256, 1, 1}, {}, 1);
    {
        const struct { size_t len; const void* data; uint32_t denom; } hadamard_f32_pipelines[] = {
            { hadamard_f32_64_len,  hadamard_f32_64_data,  32 },
            { hadamard_f32_128_len, hadamard_f32_128_data, 64 },
            { hadamard_f32_256_len, hadamard_f32_256_data, 128 },
            { hadamard_f32_512_len, hadamard_f32_512_data, 256 },
        };
        const struct { size_t len; const void* data; uint32_t denom; } hadamard_f16_pipelines[] = {
            { hadamard_f16_64_len,  hadamard_f16_64_data,  32 },
            { hadamard_f16_128_len, hadamard_f16_128_data, 64 },
            { hadamard_f16_256_len, hadamard_f16_256_data, 128 },
            { hadamard_f16_512_len, hadamard_f16_512_data, 256 },
        };
        const char* hadamard_names[] = { "hadamard_f32_64", "hadamard_f32_128", "hadamard_f32_256", "hadamard_f32_512" };
        const char* hadamard_names_f16[] = { "hadamard_f16_64", "hadamard_f16_128", "hadamard_f16_256", "hadamard_f16_512" };
        for (uint32_t i = 0; i < 4; ++i) {
            // NH/2 threads per block; the dispatch size must be a multiple of the denom
            ggml_vk_create_pipeline(device, device->pipeline_hadamard[i][0], hadamard_names[i], hadamard_f32_pipelines[i].len, hadamard_f32_pipelines[i].data, "main", 2, sizeof(vk_op_hadamard_push_constants), {hadamard_f32_pipelines[i].denom, 1, 1}, {}, 1);
            ggml_vk_create_pipeline(device, device->pipeline_hadamard[i][1], hadamard_names_f16[i], hadamard_f16_pipelines[i].len, hadamard_f16_pipelines[i].data, "main", 2, sizeof(vk_op_hadamard_push_constants), {hadamard_f16_pipelines[i].denom, 1, 1}, {}, 1);
        }
    }
    ggml_vk_create_pipeline(device, device->pipeline_fill[0], "fill_f32", fill_f32_len, fill_f32_data, "main", 1, sizeof(vk_op_push_constants), {512, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_fill[1], "fill_f16", fill_f16_len, fill_f16_data, "main", 1, sizeof(vk_op_push_constants), {512, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_get_rows_dim0[0], "get_rows_dim0_f32", get_rows_dim0_f32_len, get_rows_dim0_f32_data, "main", 3, sizeof(vk_op_get_rows_dim0_push_constants), {256, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_get_rows_dim0[1], "get_rows_dim0_f16", get_rows_dim0_f16_len, get_rows_dim0_f16_data, "main", 3, sizeof(vk_op_get_rows_dim0_push_constants), {256, 1, 1}, {}, 1);
    ...
    if (device->float_controls_rte_fp16) {
        ...
        ggml_vk_create_pipeline(device, device->pipeline_cpy_f32_quant[GGML_TYPE_Q6_0], "cpy_f32_q6_0", cpy_f32_q6_0_rte_len, cpy_f32_q6_0_rte_data, "main", 2, sizeof(vk_op_unary_push_constants), {32, 1, 1}, {}, 1);
    ...
    if (device->float_controls_rte_fp16) {
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_F32],  "set_rows_f32",  set_rows_f32_rte_len,  set_rows_f32_rte_data,  "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_F16],  "set_rows_f16",  set_rows_f16_rte_len,  set_rows_f16_rte_data,  "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_BF16], "set_rows_bf16", set_rows_bf16_rte_len, set_rows_bf16_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_Q4_0], "set_rows_q4_0", set_rows_q4_0_rte_len, set_rows_q4_0_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_Q4_1], "set_rows_q4_1", set_rows_q4_1_rte_len, set_rows_q4_1_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_Q5_0], "set_rows_q5_0", set_rows_q5_0_rte_len, set_rows_q5_0_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_Q5_1], "set_rows_q5_1", set_rows_q5_1_rte_len, set_rows_q5_1_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_Q6_0], "set_rows_q6_0", set_rows_q6_0_rte_len, set_rows_q6_0_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_Q8_0], "set_rows_q8_0", set_rows_q8_0_rte_len, set_rows_q8_0_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_IQ4_NL], "set_rows_iq4_nl", set_rows_iq4_nl_rte_len, set_rows_iq4_nl_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_F32],  "set_rows_f32_i32",  set_rows_f32_i32_rte_len,  set_rows_f32_i32_rte_data,  "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_F16],  "set_rows_f16_i32",  set_rows_f16_i32_rte_len,  set_rows_f16_i32_rte_data,  "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_BF16], "set_rows_bf16_i32", set_rows_bf16_i32_rte_len, set_rows_bf16_i32_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_Q4_0], "set_rows_q4_0_i32", set_rows_q4_0_i32_rte_len, set_rows_q4_0_i32_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_Q4_1], "set_rows_q4_1_i32", set_rows_q4_1_i32_rte_len, set_rows_q4_1_i32_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_Q5_0], "set_rows_q5_0_i32", set_rows_q5_0_i32_rte_len, set_rows_q5_0_i32_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_Q5_1], "set_rows_q5_1_i32", set_rows_q5_1_i32_rte_len, set_rows_q5_1_i32_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_Q6_0], "set_rows_q6_0_i32", set_rows_q6_0_i32_rte_len, set_rows_q6_0_i32_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_Q8_0], "set_rows_q8_0_i32", set_rows_q8_0_i32_rte_len, set_rows_q8_0_i32_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_IQ4_NL], "set_rows_iq4_nl_i32", set_rows_iq4_nl_i32_rte_len, set_rows_iq4_nl_i32_rte_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
    } else {
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_F32],  "set_rows_f32",  set_rows_f32_len,  set_rows_f32_data,  "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_F16],  "set_rows_f16",  set_rows_f16_len,  set_rows_f16_data,  "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_BF16], "set_rows_bf16", set_rows_bf16_len, set_rows_bf16_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_Q4_0], "set_rows_q4_0", set_rows_q4_0_len, set_rows_q4_0_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_Q4_1], "set_rows_q4_1", set_rows_q4_1_len, set_rows_q4_1_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_Q5_0], "set_rows_q5_0", set_rows_q5_0_len, set_rows_q5_0_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_Q5_1], "set_rows_q5_1", set_rows_q5_1_len, set_rows_q5_1_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_Q6_0], "set_rows_q6_0", set_rows_q6_0_len, set_rows_q6_0_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_Q8_0], "set_rows_q8_0", set_rows_q8_0_len, set_rows_q8_0_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[1][GGML_TYPE_IQ4_NL], "set_rows_iq4_nl", set_rows_iq4_nl_len, set_rows_iq4_nl_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_F32],  "set_rows_f32_i32",  set_rows_f32_i32_len,  set_rows_f32_i32_data,  "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_F16],  "set_rows_f16_i32",  set_rows_f16_i32_len,  set_rows_f16_i32_data,  "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_BF16], "set_rows_bf16_i32", set_rows_bf16_i32_len, set_rows_bf16_i32_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_Q4_0], "set_rows_q4_0_i32", set_rows_q4_0_i32_len, set_rows_q4_0_i32_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_Q4_1], "set_rows_q4_1_i32", set_rows_q4_1_i32_len, set_rows_q4_1_i32_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_Q5_0], "set_rows_q5_0_i32", set_rows_q5_0_i32_len, set_rows_q5_0_i32_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_Q5_1], "set_rows_q5_1_i32", set_rows_q5_1_i32_len, set_rows_q5_1_i32_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_Q6_0], "set_rows_q6_0_i32", set_rows_q6_0_i32_len, set_rows_q6_0_i32_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_Q8_0], "set_rows_q8_0_i32", set_rows_q8_0_i32_len, set_rows_q8_0_i32_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
        ggml_vk_create_pipeline(device, device->pipeline_set_rows[0][GGML_TYPE_IQ4_NL], "set_rows_iq4_nl_i32", set_rows_iq4_nl_i32_len, set_rows_iq4_nl_i32_data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
    }
...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
...
#endif
#if defined(GGML_VULKAN_COOPMAT2_DECODE_VECTOR_GLSLC_SUPPORT)
            } else if (strcmp("VK_NV_cooperative_matrix_decode_vector", properties.extensionName) == 0 &&
        ...
        fprintf(stderr, "[queue] dev=%zu single=%d cfam=%u tfam=%u qcount=%u\n", idx, device->single_queue?1:0, compute_queue_family_index, transfer_queue_family_index, queue_family_props[compute_queue_family_index].queueCount);
...
#endif
#if defined(GGML_VULKAN_COOPMAT2_DECODE_VECTOR_GLSLC_SUPPORT)
        if (coopmat2_decode_vector_support && coopmat2_decode_vector_features.cooperativeMatrixDecodeVector) {
            device->coopmat2_decode_vector = true;
        }
#endif
        }
        ...
        for (uint32_t i = 0; i < GGML_TYPE_COUNT; ++i) {
            ...
            // The q8_1 integer-dot mmq tiles follow the same vendor policy as
            // the f16-dequant tiles (they may still be disabled by the shared
            // memory checks in ggml_vk_load_shaders).
...
// IQK / KT quant families supported by the Vulkan backend (dequant + mul_mat_vec shaders,
// mat-mat uses the dequant-to-F16 fallback).
...
static bool ggml_vk_matmul_pipeline_empty(const vk_matmul_pipeline & p) {
    return p->l == nullptr && p->m == nullptr && p->s == nullptr && p->a_l == nullptr && p->a_m == nullptr && p->a_s == nullptr;
}
...
static vk_matmul_pipeline ggml_vk_get_mul_mat_mat_pipeline(ggml_backend_vk_context * ctx, ggml_type src0_type, ggml_type src1_type, ggml_prec prec) {
    ...
    // MMQ
    // The integer-dot kernels accumulate int32 dot products; f32 accumulation
    // both performs and numerically behaves better than f16 (mainline parity).
...
static vk_pipeline ggml_vk_get_dequantize_mul_mat_vec_q8_1(ggml_backend_vk_context * ctx, ggml_type a_type, uint32_t num_cols) {
    VK_LOG_DEBUG("ggml_vk_get_dequantize_mul_mat_vec_q8_1()");
    GGML_ASSERT(num_cols >= 1 && num_cols <= mul_mat_vec_max_cols);
...
static vk_matmul_pipeline ggml_vk_get_mul_mat_mat_id_pipeline(ggml_backend_vk_context * ctx, ggml_type src0_type, ggml_type src1_type, ggml_prec prec) {
    ...
    vk_matmul_pipeline p = ctx->device->fp16 ? ctx->device->pipeline_dequant_mul_mat_mat_id[src0_type].f16acc : ctx->device->pipeline_dequant_mul_mat_mat_id[src0_type].f32acc;
    return ggml_vk_matmul_pipeline_empty(p) ? nullptr : p;
}

// Push constants for the flat dequant shaders. The IQK/KT shaders read
// pc[1] = blocks per row and pc[2] = row meta bytes.
static std::vector<uint32_t> ggml_vk_dequant_push_constants(ggml_type type, uint32_t ne01, uint32_t ne10, uint32_t nel) {
    if (ggml_vk_is_iqk_type(type)) {
        return { ne01, ne10 / (uint32_t)ggml_blck_size(type), (uint32_t)ggml_internal_get_type_traits(type).row_meta_size, 0, nel };
    }
    return { ne01, ne10, ne10, ne10, nel };
}
...
static void ggml_vk_quantize_q8_1(ggml_backend_vk_context * ctx, vk_context& subctx, vk_subbuffer&& in, vk_subbuffer&& out, uint32_t ne, bool x4 = false) {
    VK_LOG_DEBUG("ggml_vk_quantize_q8_1(" << "buffer in size=" << in.buffer->size << ", buffer out size=" << out.buffer->size << ", " << ne << (x4 ? ", x4" : "") << ")");

    vk_pipeline pipeline = x4 ? ctx->device->pipeline_quantize_q8_1_x4 : ggml_vk_get_quantize_pipeline(ctx, GGML_TYPE_Q8_1);
...
static void ggml_vk_mul_mat_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, bool dryrun = false) {
    ...
    if (dryrun) {
        ...
        if (quantize_y) {
            ggml_pipeline_request_descriptor_sets(ctx, ctx->device->pipeline_quantize_q8_1_x4, 1);
        }
        if (split_k > 1) {
    ...
    if (x_non_contig) {
        ...
        const std::vector<uint32_t> pc = ggml_vk_dequant_push_constants(src0->type, (uint32_t)ne01, (uint32_t)ne10, (uint32_t)(ggml_nelements(src0)));
        ggml_vk_sync_buffers(subctx);
...
static void ggml_vk_mul_mat_vec_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, bool dryrun = false) {
    ...
    vk_pipeline dmmv = quantize_y ? ggml_vk_get_dequantize_mul_mat_vec_q8_1(ctx, src0->type, ne11) : ggml_vk_get_dequantize_mul_mat_vec(ctx, src0->type, src1->type, ne11);
    GGML_ASSERT(!qx_needs_dequant || to_fp16_vk_0 != nullptr);  // NOLINT
    GGML_ASSERT(!qy_needs_dequant || to_fp16_vk_1 != nullptr);  // NOLINT
    GGML_ASSERT(!quantize_y || to_q8_1 != nullptr);  // NOLINT
    ...
    ggml_vk_dispatch_pipeline(ctx, subctx, dmmv,
                              { vk_subbuffer{ d_X, x_buf_offset, qx_needs_dequant ? (x_sz * ne02 * ne03) : ggml_nbytes(src0) }, vk_subbuffer{ d_Y, y_buf_offset, y_sz * ne12 * ne13 }, vk_subbuffer{ d_D, d_buf_offset, d_sz * ne22 * ne23} },
...
static void ggml_vk_mul_mat_id_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * ids, ggml_tensor * dst, bool dryrun = false) {
    ...
    if (x_non_contig) {
        ...
        ggml_vk_dispatch_pipeline(ctx, subctx, to_fp16_vk_0,
            { vk_subbuffer{ d_Qx, qx_buf_offset, ggml_nbytes(src0) }, vk_subbuffer{ d_X, 0, x_sz * ne02 * ne03 } }, pc, { (uint32_t)(x_ne * ne02 * ne03), 1, 1});
    }
    if (y_non_contig) {
        ggml_vk_cpy_to_contiguous(ctx, subctx, to_fp16_vk_1, src1, { d_Qy, qy_buf_offset, VK_WHOLE_SIZE }, { d_Y, 0, VK_WHOLE_SIZE });
    }
...
static void ggml_vk_mul_mat_vec_id_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * ids, ggml_tensor * dst, bool dryrun = false) {
    ...
    const vk_mat_vec_id_push_constants pc = {
        (uint32_t)ne00, (uint32_t)(ggml_vk_is_iqk_type(src0->type) ? src0_row_meta : ne10), (uint32_t)ne10, (uint32_t)ne01,
        src0_batch_stride, stride_batch_y, (uint32_t)(ne20*ne21),
        (uint32_t)nei0, (uint32_t)ne11,
    };
    ggml_vk_sync_buffers(subctx);
    ggml_vk_dispatch_pipeline(ctx, subctx, dmmv,
        { vk_subbuffer{ d_X, x_buf_offset, qx_needs_dequant ? (x_sz * ne02 * ne03) : ggml_nbytes(src0) },
...
static void ggml_vk_flash_attn(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * q, const ggml_tensor * k, const ggml_tensor * v, const ggml_tensor * mask, const ggml_tensor * sinks, ggml_tensor * dst, bool dryrun = false) {
    ...
    // scalar is faster than coopmat2 when N==1, but only switch when the type
    // actually has a scalar pipeline (the coopmat2-only quant types do not).
    if (N == 1 && path == FA_COOPMAT2 &&
        ctx->device->pipeline_flash_attn_f32_f16[k->type][head_sizes][1][1][0] != nullptr) {
        path = FA_SCALAR;
    }
    ...
    // Sinks cannot be folded into the split-k reduce, so run a single serial
    // KV pass when they are present (the dense-with-sinks DSV4 layers).
    ...
    if (!has_sinks && workgroups_x == 1 && shader_core_count > 0) {
    ...
    ggml_backend_vk_buffer_context * s_buf_ctx = sinks ? (ggml_backend_vk_buffer_context *)sinks->buffer->context : nullptr;
    ...
    uint32_t mask_n_head_log2 = ((mask != nullptr) << 16) | ((sinks != nullptr) << 17) | n_head_log2;
        ...
        ggml_vk_dispatch_pipeline(ctx, subctx, pipeline,
                                    {
                                        ...
                                        vk_subbuffer{d_Q, 0, VK_WHOLE_SIZE},
                                    },
        ...
        ggml_vk_dispatch_pipeline(ctx, subctx, pipeline,
                                    {
                                        ...
                                        vk_subbuffer{d_M, m_buf_offset, VK_WHOLE_SIZE},
                                        vk_subbuffer{d_D, d_buf_offset, VK_WHOLE_SIZE},
                                        vk_subbuffer{sinks ? d_S : d_Q, sinks ? s_buf_offset : 0, VK_WHOLE_SIZE},
                                    },
...
static void ggml_vk_flash_attn_indexed(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * q, const ggml_tensor * k, const ggml_tensor * v, const ggml_tensor * mask, const ggml_tensor * sinks, const ggml_tensor * indices, ggml_tensor * dst, bool dryrun = false) {
    ...
    ggml_vk_sync_buffers(subctx);
    // One workgroup of 128 threads per (token, head): the threads cooperate over
    // the gathered keys (subgroup split) and the head dimension.
    ggml_vk_dispatch_pipeline(ctx, subctx, pipeline, {
...
static vk_pipeline ggml_vk_op_get_pipeline(ggml_backend_vk_context * ctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, ggml_tensor * dst, ggml_op op) {
    ...
    case GGML_OP_SET_ROWS:
        if (src1->type == GGML_TYPE_I32 || src1->type == GGML_TYPE_I64) {
            return ctx->device->pipeline_set_rows[src1->type == GGML_TYPE_I32 ? 0 : 1][dst->type];
        }
        return nullptr;
    ...
    case GGML_OP_UNARY:
        ...
        switch (ggml_get_unary_op(dst)) {
            ...
            case GGML_UNARY_OP_SQRT_SOFTPLUS:
                return ctx->device->pipeline_sqrt_softplus[dst->type == GGML_TYPE_F16];
            default:
                break;
        }
        return nullptr;
    ...
    case GGML_OP_DELTA_NET:
        if (src0->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
            ...
            switch (S_V) {
                case 16:  return ctx->device->pipeline_delta_net[0];
                case 32:  return ctx->device->pipeline_delta_net[1];
                case 64:  return ctx->device->pipeline_delta_net[2];
                case 128: return ctx->device->pipeline_delta_net[3];
...
static void ggml_vk_op_f32(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, ggml_tensor * dst, ggml_op op, PC&& pc, bool dryrun = false) {
    ...
    // Diagnostic for cross-backend scheduling bugs (e.g. a view dst whose buffer
    // belongs to a different backend): fail loudly instead of dereferencing null.
    if (d_D == nullptr) {
        fprintf(stderr, "ggml_vk_op_f32: null dst device buffer for op %s on %s (dst=%s buft=%s; src0=%s buft=%s; src1 buft=%s; src2=%p)\n",
                ggml_op_name(op), ctx->device->name.c_str(), dst->name, dst->buffer ? ggml_backend_buft_name(dst->buffer->buft) : "-",
                src0->name, src0->buffer ? ggml_backend_buft_name(src0->buffer->buft) : "-",
                use_src1 ? (src1->buffer ? ggml_backend_buft_name(src1->buffer->buft) : "-") : "-",
                use_src2 ? (void*)src2->buffer : nullptr);
        GGML_ABORT("fatal error");
    }
...
static void ggml_vk_get_rows(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, bool dryrun = false) {
    ...
    // IQK/KT rows carry a per-row scale header: pass the src0 strides in bytes
    // (the get_rows_iqk shader indexes the quantized row bytes directly).
    const uint32_t src0_stride_scale = ggml_vk_is_iqk_type(src0->type) ? 1 : src0_type_size;

    ggml_vk_op_f32<vk_op_binary_push_constants>(ctx, subctx, src0, src1, nullptr, dst, GGML_OP_GET_ROWS, {
        (uint32_t)ggml_nelements(src0),
        (uint32_t)src0->ne[0], (uint32_t)src0->ne[1], (uint32_t)src0->ne[2],(uint32_t)src0->ne[3], (uint32_t)src0->nb[0] / src0_stride_scale, (uint32_t)src0->nb[1] / src0_stride_scale, (uint32_t)src0->nb[2] / src0_stride_scale, (uint32_t)src0->nb[3] / src0_stride_scale,
...
static void ggml_vk_l2_norm(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, ggml_tensor * dst, bool dryrun = false) {
    vk_op_unary_push_constants pc = vk_op_unary_push_constants_init(src0, dst);
    pc.param1 = ggml_get_op_params_f32(dst, 0);
...
static void ggml_vk_ssm_conv(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, const ggml_tensor * src3, const ggml_tensor * src4, ggml_tensor * dst, bool dryrun = false) {
    ...
    // Single-sequence fast path: the recurrent state belongs to one sequence and the
    // per-token outputs are independent given the initial conv state.
    ...
    if (dryrun) {
        ...
        ggml_pipeline_request_descriptor_sets(ctx, ctx->device->pipeline_ssm_conv_final_state_f32, 1);
        return;
    }

    ggml_backend_vk_buffer_context * s0_ctx = (ggml_backend_vk_buffer_context *)src0->buffer->context;
    ggml_backend_vk_buffer_context * x_ctx  = (ggml_backend_vk_buffer_context *)src1->buffer->context;
    ggml_backend_vk_buffer_context * c_ctx  = (ggml_backend_vk_buffer_context *)src2->buffer->context;
    ggml_backend_vk_buffer_context * d_ctx  = (ggml_backend_vk_buffer_context *)dst->buffer->context;

    vk_buffer d_S0 = s0_ctx->dev_buffer;
    const size_t s0_off = vk_tensor_offset(src0) + src0->view_offs;
    vk_buffer d_X = x_ctx->dev_buffer;
    const size_t x_off = vk_tensor_offset(src1) + src1->view_offs;
    vk_buffer d_C = c_ctx->dev_buffer;
    const size_t c_off = vk_tensor_offset(src2) + src2->view_offs;
    ...
    ggml_vk_sync_buffers(subctx);
    ggml_vk_dispatch_pipeline(ctx, subctx, ctx->device->pipeline_ssm_conv_f32, {
        vk_subbuffer{d_S0, s0_off, VK_WHOLE_SIZE},
    ...
    ggml_vk_sync_buffers(subctx);
    ggml_vk_dispatch_pipeline(ctx, subctx, ctx->device->pipeline_ssm_conv_final_state_f32, {
        vk_subbuffer{d_S0, s0_off, VK_WHOLE_SIZE},
        vk_subbuffer{d_X, x_off, VK_WHOLE_SIZE},
        vk_subbuffer{d_D, d_off, VK_WHOLE_SIZE},
    }, pc2, { nr, 1, 1 });
...
static void ggml_vk_delta_net(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    ...
    const uint32_t H_k      = (uint32_t)q->ne[2];
    const uint32_t H_v      = (uint32_t)v->ne[2];
    const uint32_t n_tokens = (uint32_t)q->ne[1];
    const uint32_t n_seqs   = (uint32_t)q->ne[3];
    GGML_ASSERT(S_v == (uint32_t)q->ne[0]);
    ...
    const vk_op_delta_net_push_constants pc = {
        H_k, H_v, n_tokens, n_seqs,
        (uint32_t)dst->op_params[0],
        (uint32_t)(v->nb[1] / sizeof(float)),
    ...
    ggml_backend_vk_buffer_context * k_ctx     = (ggml_backend_vk_buffer_context *)k->buffer->context;
    ggml_backend_vk_buffer_context * v_ctx     = (ggml_backend_vk_buffer_context *)v->buffer->context;
    ggml_backend_vk_buffer_context * g_ctx     = (ggml_backend_vk_buffer_context *)g->buffer->context;
    ggml_backend_vk_buffer_context * beta_ctx  = (ggml_backend_vk_buffer_context *)beta->buffer->context;
    ggml_backend_vk_buffer_context * state_ctx = (ggml_backend_vk_buffer_context *)state->buffer->context;
    ggml_backend_vk_buffer_context * d_ctx     = (ggml_backend_vk_buffer_context *)dst->buffer->context;

    vk_buffer d_Q = q_ctx->dev_buffer;
    const size_t q_off = vk_tensor_offset(q) + q->view_offs;
    vk_buffer d_K = k_ctx->dev_buffer;
    const size_t k_off = vk_tensor_offset(k) + k->view_offs;
    vk_buffer d_V = v_ctx->dev_buffer;
    const size_t v_off = vk_tensor_offset(v) + v->view_offs;
    vk_buffer d_G = g_ctx->dev_buffer;
    const size_t g_off = vk_tensor_offset(g) + g->view_offs;
    vk_buffer d_B = beta_ctx->dev_buffer;
    const size_t b_off = vk_tensor_offset(beta) + beta->view_offs;
    vk_buffer d_S = state_ctx->dev_buffer;
    const size_t s_off = vk_tensor_offset(state) + state->view_offs;
    vk_buffer d_D = d_ctx->dev_buffer;
    const size_t d_off = vk_tensor_offset(dst) + dst->view_offs;
    ...
    ggml_vk_dispatch_pipeline(ctx, subctx, pipeline, {
        ...
        vk_subbuffer{d_K, k_off, VK_WHOLE_SIZE},
        vk_subbuffer{d_V, v_off, VK_WHOLE_SIZE},
        vk_subbuffer{d_G, g_off, VK_WHOLE_SIZE},
...
static void ggml_vk_sinkhorn(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    const ggml_tensor * src0 = dst->src[0];
    const uint32_t S = (uint32_t)dst->op_params[0];
    const uint32_t iters = (uint32_t)dst->op_params[1];
    float eps;
    memcpy(&eps, &dst->op_params[2], sizeof(float));
    const uint32_t transposed = (uint32_t)dst->op_params[3];
    const uint32_t T = (uint32_t)src0->ne[1];

    vk_pipeline pipeline = ctx->device->pipeline_sinkhorn_f32;
    if (dryrun) {
        ggml_pipeline_request_descriptor_sets(ctx, pipeline, 1);
        return;
    }
    ...
    const vk_op_sinkhorn_push_constants pc = {
        T, S, iters, eps, transposed,
        (uint32_t)(src0->nb[1] / sizeof(float)),
    };

    ggml_vk_sync_buffers(subctx);
    ggml_vk_dispatch_pipeline(ctx, subctx, pipeline, {
        vk_subbuffer{ src0_ctx->dev_buffer, vk_tensor_offset(src0) + src0->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ dst_ctx->dev_buffer,  vk_tensor_offset(dst) + dst->view_offs,   VK_WHOLE_SIZE },
    }, pc, { T, 1, 1 });
}

static void ggml_vk_hc_pre(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    const ggml_tensor * src0 = dst->src[0];
    ...
    ggml_backend_vk_buffer_context * src2_ctx = (ggml_backend_vk_buffer_context *)src2->buffer->context;
    ggml_backend_vk_buffer_context * dst_ctx  = (ggml_backend_vk_buffer_context *)dst->buffer->context;

    const vk_op_hc_pre_push_constants pc = {
        T, S, iters, eps,
        (uint32_t)(src0->nb[1] / sizeof(float)),
    };
    ...
    ggml_vk_dispatch_pipeline(ctx, subctx, pipeline, {
        vk_subbuffer{ src0_ctx->dev_buffer, vk_tensor_offset(src0) + src0->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ src1_ctx->dev_buffer, vk_tensor_offset(src1) + src1->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ src2_ctx->dev_buffer, vk_tensor_offset(src2) + src2->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ dst_ctx->dev_buffer,  vk_tensor_offset(dst) + dst->view_offs,   VK_WHOLE_SIZE },
    }, pc, { T, 1, 1 });
}

static void ggml_vk_hc_post(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    const ggml_tensor * x    = dst->src[0];
    const ggml_tensor * post = dst->src[1];
    const ggml_tensor * res  = dst->src[2];
    const ggml_tensor * comb = dst->src[3];

    const uint32_t S = (uint32_t)post->ne[0];
    const uint32_t ne0 = (uint32_t)x->ne[0];
    const uint32_t T = (uint32_t)x->ne[1];

    vk_pipeline pipeline = ctx->device->pipeline_hc_post_f32;
    if (dryrun) {
        ggml_pipeline_request_descriptor_sets(ctx, pipeline, 1);
        return;
    }

    ggml_backend_vk_buffer_context * x_ctx    = (ggml_backend_vk_buffer_context *)x->buffer->context;
    ggml_backend_vk_buffer_context * post_ctx = (ggml_backend_vk_buffer_context *)post->buffer->context;
    ggml_backend_vk_buffer_context * res_ctx  = (ggml_backend_vk_buffer_context *)res->buffer->context;
    ggml_backend_vk_buffer_context * comb_ctx = (ggml_backend_vk_buffer_context *)comb->buffer->context;
    ggml_backend_vk_buffer_context * dst_ctx  = (ggml_backend_vk_buffer_context *)dst->buffer->context;

    const vk_op_hc_post_push_constants pc = {
        ne0, T, S,
        (uint32_t)(x->nb[1] / sizeof(float)),
        (uint32_t)(post->nb[1] / sizeof(float)),
        (uint32_t)(comb->nb[2] / sizeof(float)),
        (uint32_t)(res->nb[1] / sizeof(float)),
        (uint32_t)(res->nb[2] / sizeof(float)),
        (uint32_t)(dst->nb[1] / sizeof(float)),
        (uint32_t)(dst->nb[2] / sizeof(float)),
    };
    ...
    ggml_vk_sync_buffers(subctx);
    ggml_vk_dispatch_pipeline(ctx, subctx, pipeline, {
        vk_subbuffer{ x_ctx->dev_buffer,    vk_tensor_offset(x) + x->view_offs,       VK_WHOLE_SIZE },
        vk_subbuffer{ post_ctx->dev_buffer, vk_tensor_offset(post) + post->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ res_ctx->dev_buffer,  vk_tensor_offset(res) + res->view_offs,   VK_WHOLE_SIZE },
        vk_subbuffer{ comb_ctx->dev_buffer, vk_tensor_offset(comb) + comb->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ dst_ctx->dev_buffer,  vk_tensor_offset(dst) + dst->view_offs,   VK_WHOLE_SIZE },
    }, pc, { ne0 * T, 1, 1 });
}

static void ggml_vk_ds4_comp(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    const ggml_tensor * state = dst->src[0];
    const ggml_tensor * score = dst->src[1];
    const ggml_tensor * idx   = dst->src[2];

    const uint32_t type = (uint32_t)dst->op_params[0];
    const uint32_t ne0 = (uint32_t)dst->ne[0];
    const uint32_t nblock = (uint32_t)dst->ne[1];
    const uint32_t ratio = type == 0
        ? (uint32_t)(idx->ne[0] / (2 * nblock))
        : (uint32_t)(idx->ne[0] / nblock);

    vk_pipeline pipeline = ctx->device->pipeline_ds4_comp_f32;
    if (dryrun) {
        ggml_pipeline_request_descriptor_sets(ctx, pipeline, 1);
        return;
    }
    ...
    ggml_backend_vk_buffer_context * state_ctx = (ggml_backend_vk_buffer_context *)state->buffer->context;
    ggml_backend_vk_buffer_context * score_ctx = (ggml_backend_vk_buffer_context *)score->buffer->context;
    ggml_backend_vk_buffer_context * idx_ctx   = (ggml_backend_vk_buffer_context *)idx->buffer->context;
    ggml_backend_vk_buffer_context * dst_ctx   = (ggml_backend_vk_buffer_context *)dst->buffer->context;

    const vk_op_ds4_comp_push_constants pc = {
        type, ne0, nblock, ratio,
        (uint32_t)(state->nb[1] / sizeof(float)),
        (uint32_t)(score->nb[1] / sizeof(float)),
    };

    ggml_vk_sync_buffers(subctx);
    ggml_vk_dispatch_pipeline(ctx, subctx, pipeline, {
        vk_subbuffer{ state_ctx->dev_buffer, vk_tensor_offset(state) + state->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ score_ctx->dev_buffer, vk_tensor_offset(score) + score->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ idx_ctx->dev_buffer,   vk_tensor_offset(idx) + idx->view_offs,     VK_WHOLE_SIZE },
        vk_subbuffer{ dst_ctx->dev_buffer,   vk_tensor_offset(dst) + dst->view_offs,     VK_WHOLE_SIZE },
    }, pc, { ne0 * nblock, 1, 1 });
}

static void ggml_vk_mask_topk(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    const ggml_tensor * mask = dst->src[0];
    const ggml_tensor * topk = dst->src[1];

    const bool f16 = mask->type == GGML_TYPE_F16;
    vk_pipeline pipeline = ctx->device->pipeline_mask_topk[f16 ? 1 : 0];
    if (dryrun) {
        ggml_pipeline_request_descriptor_sets(ctx, pipeline, 1);
        return;
    }

    const size_t mask_ts = ggml_type_size(mask->type);
    const size_t dst_ts  = ggml_type_size(dst->type);
    const uint32_t nrows = (uint32_t)ggml_nrows(dst);

    ggml_backend_vk_buffer_context * mask_ctx = (ggml_backend_vk_buffer_context *)mask->buffer->context;
    ggml_backend_vk_buffer_context * topk_ctx = (ggml_backend_vk_buffer_context *)topk->buffer->context;
    ggml_backend_vk_buffer_context * dst_ctx  = (ggml_backend_vk_buffer_context *)dst->buffer->context;

    const vk_op_mask_topk_push_constants pc = {
        (uint32_t)mask->ne[0],
        (uint32_t)topk->ne[0],
        (uint32_t)topk->ne[1],
        (uint32_t)mask->ne[1],
        (uint32_t)mask->ne[2],
        (uint32_t)mask->ne[3],
        (uint32_t)(mask->nb[1] / mask_ts),
        (uint32_t)(mask->nb[2] / mask_ts),
        (uint32_t)(mask->nb[3] / mask_ts),
        (uint32_t)(topk->nb[1] / sizeof(int32_t)),
        (uint32_t)(topk->nb[2] / sizeof(int32_t)),
        (uint32_t)(topk->nb[3] / sizeof(int32_t)),
        (uint32_t)(dst->nb[1] / dst_ts),
        (uint32_t)(dst->nb[2] / dst_ts),
        (uint32_t)(dst->nb[3] / dst_ts),
    };

    ggml_vk_sync_buffers(subctx);
    ggml_vk_dispatch_pipeline(ctx, subctx, pipeline, {
        vk_subbuffer{ mask_ctx->dev_buffer, vk_tensor_offset(mask) + mask->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ topk_ctx->dev_buffer, vk_tensor_offset(topk) + topk->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ dst_ctx->dev_buffer,  vk_tensor_offset(dst) + dst->view_offs,   VK_WHOLE_SIZE },
    }, pc, { nrows * 256u, 1, 1 });
}

static void ggml_vk_mul_multi_add(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    const ggml_tensor * src0 = dst->src[0];
    const ggml_tensor * src1 = dst->src[1];
    const ggml_tensor * src2 = dst->src[2];  // optional f32 scales
    const ggml_tensor * src3 = dst->src[3];  // optional i32 expert ids

    vk_pipeline pipeline = ctx->device->pipeline_mul_multi_add_f32;
    if (dryrun) {
        ggml_pipeline_request_descriptor_sets(ctx, pipeline, 1);
        return;
    }
    ...
    ggml_backend_vk_buffer_context * src0_ctx = (ggml_backend_vk_buffer_context *)src0->buffer->context;
    ggml_backend_vk_buffer_context * src1_ctx = (ggml_backend_vk_buffer_context *)src1->buffer->context;
    ggml_backend_vk_buffer_context * src2_ctx = src2 ? (ggml_backend_vk_buffer_context *)src2->buffer->context : nullptr;
    ggml_backend_vk_buffer_context * src3_ctx = src3 ? (ggml_backend_vk_buffer_context *)src3->buffer->context : nullptr;
    ggml_backend_vk_buffer_context * dst_ctx  = (ggml_backend_vk_buffer_context *)dst->buffer->context;

    const vk_op_mul_multi_add_push_constants pc = {
        (uint32_t)src0->ne[0],
        (uint32_t)src0->ne[1],
        (uint32_t)src0->ne[2],
        (uint32_t)(src0->nb[1] / sizeof(float)),
        ...
        src3 ? (uint32_t)(src3->nb[1] / sizeof(int32_t)) : 0u,
        (uint32_t)(dst->nb[1] / sizeof(float)),
        src2 && src3 ? 1u : 0u,
    };

    // Bind every optional slot to a valid buffer (never read when the operand is null).
    vk_buffer dummy = src0_ctx->dev_buffer;
    ggml_vk_sync_buffers(subctx);
    ggml_vk_dispatch_pipeline(ctx, subctx, pipeline, {
        vk_subbuffer{ src0_ctx->dev_buffer, vk_tensor_offset(src0) + src0->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ src1_ctx->dev_buffer, vk_tensor_offset(src1) + src1->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ src2_ctx ? src2_ctx->dev_buffer : dummy, src2 ? vk_tensor_offset(src2) + src2->view_offs : 0, VK_WHOLE_SIZE },
        vk_subbuffer{ src3_ctx ? src3_ctx->dev_buffer : dummy, src3 ? vk_tensor_offset(src3) + src3->view_offs : 0, VK_WHOLE_SIZE },
        vk_subbuffer{ dst_ctx->dev_buffer,  vk_tensor_offset(dst) + dst->view_offs,  VK_WHOLE_SIZE },
    }, pc, { (uint32_t)(src0->ne[0] * src0->ne[2]), 1, 1 });
}

static void ggml_vk_hadamard(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    const ggml_tensor * src = dst->src[0];

    const uint32_t nh = (uint32_t)dst->op_params[0];
    uint32_t idx;
    switch (nh) {
        case 64:  idx = 0; break;
        case 128: idx = 1; break;
        case 256: idx = 2; break;
        case 512: idx = 3; break;
        default: GGML_ABORT("hadamard: unsupported block size %u", nh);
    }
    const bool f16 = src->type == GGML_TYPE_F16;
    vk_pipeline pipeline = ctx->device->pipeline_hadamard[idx][f16 ? 1 : 0];
    if (dryrun) {
        ggml_pipeline_request_descriptor_sets(ctx, pipeline, 1);
        return;
    }

    const size_t src_ts = ggml_type_size(src->type);
    const uint32_t nc = (uint32_t)(src->ne[0] / nh);

    ggml_backend_vk_buffer_context * src_ctx = (ggml_backend_vk_buffer_context *)src->buffer->context;
    ggml_backend_vk_buffer_context * dst_ctx = (ggml_backend_vk_buffer_context *)dst->buffer->context;

    const vk_op_hadamard_push_constants pc = {
        nc,
        (uint32_t)(src->nb[1] / src_ts),
        (uint32_t)(src->nb[2] / src_ts),
        (uint32_t)(src->nb[3] / src_ts),
        (uint32_t)(dst->nb[1] / sizeof(float)),
        (uint32_t)(dst->nb[2] / sizeof(float)),
    ...
    ggml_vk_sync_buffers(subctx);
    ggml_vk_dispatch_pipeline(ctx, subctx, pipeline, {
        vk_subbuffer{ src_ctx->dev_buffer, vk_tensor_offset(src) + src->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ dst_ctx->dev_buffer, vk_tensor_offset(dst) + dst->view_offs, VK_WHOLE_SIZE },
    }, pc, { nc * (uint32_t)src->ne[1] * (nh/2u), (uint32_t)src->ne[2], (uint32_t)src->ne[3] });
}

static void ggml_vk_fill(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    vk_pipeline pipeline = ctx->device->pipeline_fill[dst->type == GGML_TYPE_F16 ? 1 : 0];
    if (dryrun) {
        ggml_pipeline_request_descriptor_sets(ctx, pipeline, 1);
        return;
    }
    ...
    float value;
    memcpy(&value, &dst->op_params[0], sizeof(float));

    ggml_backend_vk_buffer_context * dst_ctx = (ggml_backend_vk_buffer_context *)dst->buffer->context;

    // Same dispatch shape as the generic elementwise ops in ggml_vk_op_f32
    // (512 x 512 x z grid addressing).
    const uint32_t n = (uint32_t)ggml_nelements(dst);
    std::array<uint32_t, 3> elements;
    if (n > 262144u) {
        elements = { 512u, 512u, CEIL_DIV(n, 262144u) };
    } else if (n > 512u) {
        elements = { 512u, CEIL_DIV(n, 512u), 1u };
    } else {
        elements = { n, 1u, 1u };
    }
    ...
    ggml_vk_dispatch_pipeline(ctx, subctx, pipeline, {
        vk_subbuffer{ dst_ctx->dev_buffer, vk_tensor_offset(dst) + dst->view_offs, VK_WHOLE_SIZE },
    }, vk_op_push_constants{ n, 0, value, 0.0f }, elements);
}

static void ggml_vk_get_rows_dim0(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    const ggml_tensor * src = dst->src[0];
    const ggml_tensor * ids = dst->src[1];

    const bool f16 = src->type != GGML_TYPE_F32;
    vk_pipeline pipeline = ctx->device->pipeline_get_rows_dim0[f16 ? 1 : 0];
    if (dryrun) {
        ggml_pipeline_request_descriptor_sets(ctx, pipeline, 1);
        return;
    }
    ...
    const vk_op_get_rows_dim0_push_constants pc = {
        (uint32_t)dst->ne[0],
        (uint32_t)dst->ne[1],
        (uint32_t)dst->ne[2],
        (uint32_t)dst->ne[3],
        ...
        (uint32_t)(ids->nb[2] / sizeof(int32_t)),
        (uint32_t)(ids->nb[3] / sizeof(int32_t)),
        (uint32_t)(dst->nb[1] / dst_ts),
        (uint32_t)(dst->nb[2] / dst_ts),
        (uint32_t)(dst->nb[3] / dst_ts),
    };
    ...
    ggml_vk_dispatch_pipeline(ctx, subctx, pipeline, {
        vk_subbuffer{ src_ctx->dev_buffer, vk_tensor_offset(src) + src->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ ids_ctx->dev_buffer, vk_tensor_offset(ids) + ids->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ dst_ctx->dev_buffer, vk_tensor_offset(dst) + dst->view_offs, VK_WHOLE_SIZE },
    }, pc, { total, 1, 1 });
}

static void ggml_vk_mask_to_idx(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    const ggml_tensor * src = dst->src[0];

    const bool f16 = src->type == GGML_TYPE_F16;
    vk_pipeline pipeline = ctx->device->pipeline_mask_to_idx[f16 ? 1 : 0];
    if (dryrun) {
        ggml_pipeline_request_descriptor_sets(ctx, pipeline, 1);
        return;
    }
    ...
    const vk_op_mask_to_idx_push_constants pc = {
        (uint32_t)src->ne[0],
        (uint32_t)dst->ne[0],
        (uint32_t)src->ne[1],
        (uint32_t)src->ne[2],
        ...
        (uint32_t)(src->nb[2] / src_ts),
        (uint32_t)(src->nb[3] / src_ts),
        (uint32_t)(dst->nb[1] / sizeof(int32_t)),
        (uint32_t)(dst->nb[2] / sizeof(int32_t)),
        (uint32_t)(dst->nb[3] / sizeof(int32_t)),
    };
...
static void ggml_vk_indexer_topk(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    const ggml_tensor * k    = dst->src[0];
    const ggml_tensor * q    = dst->src[1];
    const ggml_tensor * w    = dst->src[2];
    const ggml_tensor * mask = dst->src[3];

    const bool f16 = k->type == GGML_TYPE_F16;
    const uint32_t n_kv    = (uint32_t)k->ne[1];
    const uint32_t n_rows  = (uint32_t)q->ne[2];
    const uint32_t n_top_k = (uint32_t)dst->ne[0];

    vk_pipeline score_pipeline   = ctx->device->pipeline_indexer_topk_score[f16 ? 1 : 0][mask->type == GGML_TYPE_F16 ? 1 : 0];
    // Prefer the shared-memory bitonic select (big-k fast path) when the row fits
    // and the pipeline exists (created only on devices with >= 36 KiB shmem).
    vk_pipeline select_pipeline  = ctx->device->pipeline_indexer_topk_select;
    if (n_kv <= 4096u && ctx->device->pipeline_indexer_topk_select_bitonic &&
        ctx->device->pipeline_indexer_topk_select_bitonic->compiled) {
        select_pipeline = ctx->device->pipeline_indexer_topk_select_bitonic;
    }
    if (dryrun) {
        ggml_pipeline_request_descriptor_sets(ctx, score_pipeline, 1);
        ggml_pipeline_request_descriptor_sets(ctx, select_pipeline, 1);
        return;
    }

    // Bound the score scratch buffer: tile query rows so n_kv * tile_rows stays small.
    const uint32_t max_score_elems = 1u << 22; // 16 MiB of floats
    uint32_t max_rows = max_score_elems / n_kv;
    if (max_rows < 1u) max_rows = 1u;
    if (max_rows > n_rows) max_rows = n_rows;

    vk_buffer score_buf = ggml_vk_create_buffer_temp(ctx, (size_t)n_kv * max_rows * sizeof(float));

    ggml_backend_vk_buffer_context * k_ctx    = (ggml_backend_vk_buffer_context *)k->buffer->context;
    ggml_backend_vk_buffer_context * q_ctx    = (ggml_backend_vk_buffer_context *)q->buffer->context;
    ggml_backend_vk_buffer_context * w_ctx    = (ggml_backend_vk_buffer_context *)w->buffer->context;
    ggml_backend_vk_buffer_context * mask_ctx = (ggml_backend_vk_buffer_context *)mask->buffer->context;
    ggml_backend_vk_buffer_context * dst_ctx  = (ggml_backend_vk_buffer_context *)dst->buffer->context;

    const uint64_t k_off    = vk_tensor_offset(k) + k->view_offs;
    const uint64_t q_off    = vk_tensor_offset(q) + q->view_offs;
    const uint64_t w_off    = vk_tensor_offset(w) + w->view_offs;
    const uint64_t mask_off = vk_tensor_offset(mask) + mask->view_offs;
    const uint64_t dst_off  = vk_tensor_offset(dst) + dst->view_offs;

    const uint32_t k_nb1    = (uint32_t)(k->nb[1] / ggml_type_size(k->type));
    const uint32_t q_nb1    = (uint32_t)(q->nb[1] / sizeof(float));
    const uint32_t q_nb2    = (uint32_t)(q->nb[2] / sizeof(float));
    const uint32_t w_nb1    = (uint32_t)(w->nb[1] / sizeof(float));
    const uint32_t m_nb1    = (uint32_t)(mask->nb[1] / ggml_type_size(mask->type));
    const uint32_t dst_nb1  = (uint32_t)(dst->nb[1] / sizeof(int32_t));

    for (uint32_t first = 0; first < n_rows; first += max_rows) {
        const uint32_t tile_rows = std::min(max_rows, n_rows - first);

        GGML_ASSERT(score_pipeline && score_pipeline->compiled);
        GGML_ASSERT(select_pipeline && select_pipeline->compiled);

        const vk_op_indexer_topk_score_push_constants pc_score = {
            n_kv, (uint32_t)k->ne[0], (uint32_t)q->ne[1], tile_rows, first,
            k_nb1, q_nb1, q_nb2, w_nb1, m_nb1,
        };
        ggml_vk_sync_buffers(subctx);
        ggml_vk_dispatch_pipeline(ctx, subctx, score_pipeline, {
            vk_subbuffer{ k_ctx->dev_buffer,    k_off,    VK_WHOLE_SIZE },
            vk_subbuffer{ q_ctx->dev_buffer,    q_off,    VK_WHOLE_SIZE },
            vk_subbuffer{ w_ctx->dev_buffer,    w_off,    VK_WHOLE_SIZE },
            vk_subbuffer{ mask_ctx->dev_buffer, mask_off, VK_WHOLE_SIZE },
            vk_subbuffer{ score_buf,            0,        VK_WHOLE_SIZE },
        }, pc_score, { n_kv * tile_rows, 1, 1 });

        const vk_op_indexer_topk_select_push_constants pc_select = {
            n_kv, n_top_k, first, dst_nb1,
        };
        ggml_vk_sync_buffers(subctx);
        ggml_vk_dispatch_pipeline(ctx, subctx, select_pipeline, {
            vk_subbuffer{ score_buf,          0,      VK_WHOLE_SIZE },
            vk_subbuffer{ dst_ctx->dev_buffer, dst_off, VK_WHOLE_SIZE },
        }, pc_select, { tile_rows * 256u, 1, 1 });
    }
}

static void ggml_vk_latent_attn(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst, bool dryrun = false) {
    const ggml_tensor * q        = dst->src[0];
    ...
    int cache_idx;
    switch (cache->type) {
        case GGML_TYPE_F32:  cache_idx = 0; break;
        case GGML_TYPE_F16:  cache_idx = 1; break;
        case GGML_TYPE_Q8_0: cache_idx = 2; break;
        default: GGML_ABORT("fatal error");
    }
    vk_pipeline pipeline = ctx->device->pipeline_latent_attn[cache_idx];
    if (dryrun) {
        ggml_pipeline_request_descriptor_sets(ctx, pipeline, 1);
        return;
    }
    ...
    const uint32_t dv     = (uint32_t)dst->op_params[2];
    const uint32_t dv_off = (uint32_t)dst->op_params[3];
    const uint32_t mode   = (uint32_t)dst->op_params[4];
    const uint32_t P      = pk ? (uint32_t)pk->ne[1] : 0u;
    const uint32_t T      = (uint32_t)q->ne[1];
    const uint32_t H      = (uint32_t)q->ne[2];
    const uint32_t N      = (uint32_t)cache->ne[1];
    const uint32_t topk   = indices ? (uint32_t)indices->ne[0] : 0u;

    ggml_backend_vk_buffer_context * q_ctx       = (ggml_backend_vk_buffer_context *)q->buffer->context;
    ggml_backend_vk_buffer_context * cache_ctx   = (ggml_backend_vk_buffer_context *)cache->buffer->context;
    ggml_backend_vk_buffer_context * pk_ctx      = pk ? (ggml_backend_vk_buffer_context *)pk->buffer->context : nullptr;
    ggml_backend_vk_buffer_context * pv_ctx      = pv ? (ggml_backend_vk_buffer_context *)pv->buffer->context : nullptr;
    ggml_backend_vk_buffer_context * mask_ctx    = mask ? (ggml_backend_vk_buffer_context *)mask->buffer->context : nullptr;
    ggml_backend_vk_buffer_context * indices_ctx = indices ? (ggml_backend_vk_buffer_context *)indices->buffer->context : nullptr;
    ggml_backend_vk_buffer_context * dst_ctx     = (ggml_backend_vk_buffer_context *)dst->buffer->context;

    const vk_op_latent_attn_push_constants pc = {
        (uint32_t)q->ne[0],
        T, H, N, P, topk,
        dv, dv_off, mode,
        scale,
        (uint32_t)(cache->nb[1] / ggml_type_size(cache->type)),
        (uint32_t)(q->nb[1] / sizeof(float)),
        (uint32_t)(q->nb[2] / sizeof(float)),
        pk ? (uint32_t)(pk->nb[1] / sizeof(float)) : 0u,
        pv ? (uint32_t)(pv->nb[1] / sizeof(float)) : 0u,
        mask ? (uint32_t)(mask->nb[1] / sizeof(float)) : 0u,
        mask ? 1u : 0u,
        indices ? (uint32_t)(indices->nb[1] / sizeof(int32_t)) : 0u,
        (uint32_t)(dst->nb[1] / sizeof(float)),
        (uint32_t)(dst->nb[2] / sizeof(float)),
    };

    // Bind every optional slot to a valid buffer (the shader never reads them when
    // the corresponding operand is null, but the descriptor must not be VK_NULL_HANDLE).
    vk_buffer dummy = q_ctx->dev_buffer;
    ggml_vk_sync_buffers(subctx);
    ggml_vk_dispatch_pipeline(ctx, subctx, pipeline, {
        vk_subbuffer{ q_ctx->dev_buffer, vk_tensor_offset(q) + q->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ cache_ctx->dev_buffer, vk_tensor_offset(cache) + cache->view_offs, VK_WHOLE_SIZE },
        vk_subbuffer{ pk_ctx ? pk_ctx->dev_buffer : dummy, pk ? vk_tensor_offset(pk) + pk->view_offs : 0, VK_WHOLE_SIZE },
        vk_subbuffer{ pv_ctx ? pv_ctx->dev_buffer : dummy, pv ? vk_tensor_offset(pv) + pv->view_offs : 0, VK_WHOLE_SIZE },
        vk_subbuffer{ mask_ctx ? mask_ctx->dev_buffer : dummy, mask ? vk_tensor_offset(mask) + mask->view_offs : 0, VK_WHOLE_SIZE },
        vk_subbuffer{ indices_ctx ? indices_ctx->dev_buffer : dummy, indices ? vk_tensor_offset(indices) + indices->view_offs : 0, VK_WHOLE_SIZE },
        vk_subbuffer{ dst_ctx->dev_buffer, vk_tensor_offset(dst) + dst->view_offs, VK_WHOLE_SIZE },
    }, pc, { T * H, 1, 1 });
}
...
static void ggml_vk_test_dequant_matmul(ggml_backend_vk_context * ctx, size_t m, size_t n, size_t k, size_t batch, size_t num_it, size_t split_k, size_t shader_size, ggml_type quant, bool mmq = false) {
    ...
    if (mmq) {
        ggml_pipeline_request_descriptor_sets(ctx, ctx->device->pipeline_quantize_q8_1_x4, num_it);
    }
    ...
    if (mmq) {
        for (size_t i = 0; i < num_it; i++) {
            ggml_vk_quantize_q8_1(ctx, subctx, { y_buf, 0, y_sz }, { qy_buf, 0, qy_sz }, y_ne, true);
            ggml_vk_matmul(
...
static bool ggml_vk_cuda_probe(vk_device dev) {
    ...
    if (g_cuda.cuMemcpyHtoD_v2(p, magic, sizeof(magic)) != CUDA_SUCCESS) {
        return false;
    }
    ...
    float vk_read[4] = { 0 };
    ggml_vk_buffer_read(probe, 0, vk_read, sizeof(vk_read));
    return memcmp(magic, vk_read, sizeof(magic)) == 0;
}

// Per-device device-local staging buffer for the CUDA P2P reduce. The CUDA copy engine
// writes/reads cuda_staging.
...
static void ggml_vk_op_reduce_impl(ggml_backend_vk_context * ctx, ggml_tensor * node) {
    ...
    // cross-device copy. This is opt-in (GGML_VK_CUDA_P2P=1) because the CUDA copy
    // engine and the Vulkan compute engine do not share a coherent cache domain on
    // NVIDIA, and the cross-engine flush needed around the copies is not yet wired up.
    ggml_vk_cuda_init();
    const bool want_cuda_p2p = getenv("GGML_VK_CUDA_P2P") != nullptr;
    if (want_cuda_p2p && g_cuda.loaded && !multi_remote && remote_src && node->buffer && remote_src->buffer &&
        ...
        if (g_cuda.loaded && want_cuda_p2p && local_dev && remote_dev && local_dev != remote_dev) {
...
static bool ggml_vk_build_graph(ggml_backend_vk_context * ctx, ggml_cgraph * cgraph, int node_idx, ggml_tensor *node_begin, int node_idx_begin, bool dryrun, bool last_node, bool almost_ready, bool submit){
    ...
    if (!dryrun) {
        ...
        case GGML_OP_GET_ROWS:
            {
                // row gather (generic pipeline) or the "dim0" per-element gather
                vk_pipeline pipeline = node->op_params[0] == 1
                    ? ctx->device->pipeline_get_rows_dim0[node->src[0]->type != GGML_TYPE_F32 ? 1 : 0]
                    : ggml_vk_op_get_pipeline(ctx, src0, src1, src2, node, node->op);
                if (!pipeline) {
    ...
    case GGML_OP_ACC:
        ...
        break;
    case GGML_OP_GET_ROWS:
        if (node->op_params[0] == 1) {
            // "dim0" per-element gather (ggml_get_rows_ext dim0=true)
            ggml_vk_get_rows_dim0(ctx, compute_ctx, node, dryrun);
        } else {
    ...
    case GGML_OP_FLASH_ATTN_EXT:
        if (src5 != nullptr) {
            ...
            ggml_vk_flash_attn(ctx, compute_ctx, src0, src1, src2, src3, src4, node, dryrun);
        }
    ...
    case GGML_OP_SINKHORN:
        ggml_vk_sinkhorn(ctx, compute_ctx, node, dryrun);
        break;

    case GGML_OP_HC_PRE:
        ggml_vk_hc_pre(ctx, compute_ctx, node, dryrun);
        break;

    case GGML_OP_HC_POST:
        ggml_vk_hc_post(ctx, compute_ctx, node, dryrun);
        break;

    case GGML_OP_DS4_COMP:
        ggml_vk_ds4_comp(ctx, compute_ctx, node, dryrun);
        break;

    case GGML_OP_MASK_TOPK:
        ggml_vk_mask_topk(ctx, compute_ctx, node, dryrun);
        break;

    case GGML_OP_MASK_TO_IDX:
        ggml_vk_mask_to_idx(ctx, compute_ctx, node, dryrun);
        break;

    case GGML_OP_INDEXER_TOPK:
        ggml_vk_indexer_topk(ctx, compute_ctx, node, dryrun);
        break;

    case GGML_OP_MUL_MULTI_ADD:
        ggml_vk_mul_multi_add(ctx, compute_ctx, node, dryrun);
        break;

    case GGML_OP_HADAMARD:
        ggml_vk_hadamard(ctx, compute_ctx, node, dryrun);
        break;

    case GGML_OP_FILL:
        ggml_vk_fill(ctx, compute_ctx, node, dryrun);
        break;

    case GGML_OP_LATENT_ATTN:
        ggml_vk_latent_attn(ctx, compute_ctx, node, dryrun);
        break;
...
// Fingerprint of the graph content that determines the dryrun result (which
// pipelines are requested and the matmul-byte estimate). The cache is keyed on
// the cgraph pointer for speed (llama graph_reuse re-submits the same object),
// but a freed cgraph can be reallocated at the same address with a different
// graph. Hashing the node/source metadata (and the tensor identities, for the
// RMS_NORM+MUL fusion topology) makes a stale pointer match fall through to a
// fresh dryrun.
static uint64_t ggml_vk_hash_mix(uint64_t h, uint64_t v) {
    return (h ^ (v + 0x9e3779b97f4a7c15ULL + (h << 6) + (h >> 2))) * 0x9e3779b97f4a7c15ULL;
}

static uint64_t ggml_vk_tensor_hash(uint64_t h, const ggml_tensor * t) {
    if (t == nullptr) {
        return ggml_vk_hash_mix(h, 0xFFFFFFFFFFFFFFFFULL);
    }
    h = ggml_vk_hash_mix(h, (uint64_t)(uintptr_t)t);
    h = ggml_vk_hash_mix(h, (uint64_t)t->op);
    h = ggml_vk_hash_mix(h, (uint64_t)t->type);
    for (int i = 0; i < GGML_MAX_DIMS; ++i) {
        h = ggml_vk_hash_mix(h, (uint64_t)t->ne[i]);
        h = ggml_vk_hash_mix(h, (uint64_t)t->nb[i]);
    }
    for (int i = 0; i < GGML_MAX_OP_PARAMS / (int)sizeof(int32_t); ++i) {
        h = ggml_vk_hash_mix(h, (uint64_t)(uint32_t)t->op_params[i]);
    }
    return h;
}

static uint64_t ggml_vk_cgraph_hash(const ggml_cgraph * cgraph) {
    uint64_t h = 0x9e3779b97f4a7c15ULL;
    h = ggml_vk_hash_mix(h, (uint64_t)cgraph->n_nodes);
    for (int i = 0; i < cgraph->n_nodes; ++i) {
        const ggml_tensor * node = cgraph->nodes[i];
        h = ggml_vk_tensor_hash(h, node);
        for (int s = 0; s < GGML_MAX_SRC; ++s) {
            h = ggml_vk_tensor_hash(h, node->src[s]);
        }
    }
    return h;
}
...
static ggml_status ggml_backend_vk_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    const uint64_t cgraph_hash = ggml_vk_cgraph_hash(cgraph);
    if (ctx->cached_cgraph == cgraph && ctx->cached_n_nodes == cgraph->n_nodes && ctx->cached_cgraph_hash == cgraph_hash) {
        ...
        ctx->cached_cgraph_hash = cgraph_hash;
    ...
    if (vk_perf_logger_enabled) {
        ...
        if (!ctx->compute_ctx.expired()) {
            // Flush any pending compute so the timestamp queries are ordered.
            vk_context pending = ctx->compute_ctx.lock();
            ggml_vk_ctx_end(pending);
            ggml_vk_submit(pending, ctx->device->fence);
            VK_CHECK(ctx->device->device.waitForFences({ ctx->device->fence }, true, UINT64_MAX), "perf_logger waitForFences");
...
static bool ggml_backend_vk_supports_op(ggml_backend_t backend, const ggml_tensor * op) {
    switch (op->op) {
        ...
        case GGML_OP_MUL_MAT_ID:
            {
                ...
                if (op->op == GGML_OP_MUL_MAT_ID) {
                    // IQK/KT quants use the dequant-to-F16 fallback for the
                    // mat-mat-id path (no native mat_mat_id pipelines). MXFP4
                    // now has a native cm2 matmul_id.
                    if (!ggml_vk_is_iqk_type(src0_type)) {
        ...
        case GGML_OP_FLASH_ATTN_EXT:
            {
                ...
                if (op->src[5] != NULL) {
                    ...
                    // The cooperative (subgroup-split) kernel keeps the per-subgroup
                    // value accumulators in shared memory (8 subgroups x Dv floats,
                    // Dv <= 512) and assumes >= 16 lanes per subgroup.
                    if (op->src[2]->ne[0] > 512) {
                        return false;
                    }
                    if (device->subgroup_size < 16) {
                        return false;
                    }
                    if (device->properties.limits.maxComputeSharedMemorySize < 17u*1024u) {
                        return false;
                    }
                ...
                // Dense FA with per-head attention sinks (src[4]) is supported
                // in the scalar (N==1) and cm2 FA paths. The cm1 path has no
                // sink support, so keep the prompt (N>1) case on CPU there.
                if (op->src[4] != NULL) {
                    if (op->src[4]->type != GGML_TYPE_F32) {
                        return false;
                    }
                    if (!device->coopmat2 && op->src[0]->ne[1] > 1) {
                        return false;
                    }
                }
                ...
                case GGML_TYPE_Q6_0:
                case GGML_TYPE_Q8_0:
                    // supported in scalar, coopmat1 and coopmat2 paths
                    break;
                case GGML_TYPE_Q4_1:
                case GGML_TYPE_Q5_0:
                case GGML_TYPE_Q5_1:
        ...
        case GGML_OP_DELTA_NET:
            {
                ...
                if (op->src[0]->ne[0] != S_v) {
                    return false;
                }
                if (op->src[2]->ne[2] % op->src[0]->ne[2] != 0) {
                    return false;
                }
                return true;
            }
        ...
        case GGML_OP_MASK_TOPK:
            {
                ...
                if ((t != GGML_TYPE_F32 && t != GGML_TYPE_F16) || op->src[1]->type != GGML_TYPE_I32 ||
                    op->type != t) {
                    return false;
                }
                return true;
        ...
        case GGML_OP_MASK_TO_IDX:
            {
                const ggml_type t = op->src[0]->type;
                if ((t != GGML_TYPE_F32 && t != GGML_TYPE_F16) || op->type != GGML_TYPE_I32) {
                    return false;
                }
                return true;
            }
        case GGML_OP_INDEXER_TOPK:
            {
                if (ggml_unary_op(op->op_params[0]) != GGML_UNARY_OP_RELU) {
                    return false;
                }
                const ggml_type kt = op->src[0]->type;
                const ggml_type mt = op->src[3]->type;
                if ((kt != GGML_TYPE_F32 && kt != GGML_TYPE_F16) || op->src[1]->type != GGML_TYPE_F32 ||
                    op->src[2]->type != GGML_TYPE_F32 ||
                    (mt != GGML_TYPE_F32 && mt != GGML_TYPE_F16) ||
                    op->type != GGML_TYPE_I32) {
                    return false;
                }
                // Real DSA checkpoints configure indexer top_k in the hundreds to
                // thousands (DSV4-Flash uses 512). The select shader handles any k
                // (bitonic sort of the row for n_kv <= 4096, threshold-rescan
                // tournament beyond), so there is no small-k gate anymore.
                if (op->src[0]->ne[1] <= op->ne[0]) {
                    return false;
                }
                return true;
            }
        case GGML_OP_MUL_MULTI_ADD:
            {
                // Hyper-connection mixing (hc_attn_pre/hc_ffn_pre) and the fused MoE
                // weighted sum. All operands f32; src2/src3 (per-expert scales/ids)
                // are optional.
                if (op->src[0]->type != GGML_TYPE_F32 || op->src[1]->type != GGML_TYPE_F32 ||
                    op->type != GGML_TYPE_F32) {
                    return false;
                }
                if (op->src[0]->ne[3] != 1 || op->src[1]->ne[0] != 1) {
                    return false;
                }
                if (op->src[0]->ne[1] != op->src[1]->ne[1] ||
                ...
                if (op->src[2] != nullptr || op->src[3] != nullptr) {
                    ...
                    if (op->src[2]->type != GGML_TYPE_F32 || op->src[3]->type != GGML_TYPE_I32) {
                        return false;
                    }
                    if (op->src[3]->ne[0] != op->src[0]->ne[1]) {
                        return false;
                    }
                }
                return true;
        ...
        case GGML_OP_HADAMARD:
            {
                if (op->type != GGML_TYPE_F32 || !ggml_are_same_shape(op->src[0], op)) {
                    return false;
                }
                if (op->src[0]->type != GGML_TYPE_F32 && op->src[0]->type != GGML_TYPE_F16) {
                    return false;
                }
                const int64_t nh = op->op_params[0];
                if (nh != 64 && nh != 128 && nh != 256 && nh != 512) {
                    return false;
                }
                if (op->ne[0] % nh != 0) {
                    return false;
        ...
        case GGML_OP_LATENT_ATTN:
            {
                ...
                const ggml_type ct = op->src[1]->type;
                if (ct != GGML_TYPE_F32 && ct != GGML_TYPE_F16 && ct != GGML_TYPE_Q8_0) {
                    return false;
                }
                if (op->src[2] && op->src[2]->type != GGML_TYPE_F32) {
                    return false;
                }
                ...
                if (op->src[5] && op->src[5]->type != GGML_TYPE_I32) {
                    return false;
                }
                const int64_t mode = op->op_params[4];
                if (mode != 0 && mode != 1) {
                    return false;
                }
                return true;
        ...
        case GGML_OP_GET_ROWS:
            {
                // The generic get_rows shaders implement the row-gather form. The
                // "dim0" variant (ggml_get_rows_ext with op_params[0] == 1, used by
                // the DSV4 single-token CSA mask gather) is a dedicated per-element
                // gather supporting the 2- and 4-byte types.
                if (op->op_params[0] == 1) {
                    const ggml_type t = op->src[0]->type;
                    if (t != GGML_TYPE_F32 && t != GGML_TYPE_F16 && t != GGML_TYPE_BF16) {
                        return false;
                    }
                    if (op->src[1]->type != GGML_TYPE_I32 || op->type != t) {
                        return false;
                    }
                    // same trailing dims; gathered dim0 count must fit
                    if (op->src[0]->ne[1] != op->src[1]->ne[1] ||
                        op->src[0]->ne[2] != op->src[1]->ne[2] ||
        ...
        case GGML_OP_GROUP_NORM:
            return ggml_is_contiguous(op->src[0]);
        case GGML_OP_L2_NORM:
            // stride-aware shader; the q/k permuted views for prompt batches are not fully contiguous
            return op->src[0]->type == GGML_TYPE_F32 && op->type == GGML_TYPE_F32;
        case GGML_OP_ADD:
        case GGML_OP_SUB:
        case GGML_OP_MUL:
...
static void ggml_vk_check_results_0(ggml_backend_vk_context * ctx, ggml_cgraph * cgraph, int tensor_idx) {
    ...
    if (tensor->op == GGML_OP_FLASH_ATTN_EXT) {
        ...
        case GGML_UNARY_OP_SOFTPLUS:
            ...
            break;
        case GGML_UNARY_OP_SQRT_SOFTPLUS:
            tensor_clone = ggml_sqrt_softplus(ggml_ctx, src_clone[0]);
            break;
        default:
