...
// Usage: test-iqk-cm2-time <backend> <type> <k> <m> [n]
#include <ggml.h>
...
int main(int argc, char ** argv) {
    ...
    printf("TIME %s [%lld,%lld]x[%lld,%lld]: %.2f ms (%.0f TFLOPS)\n", ggml_type_name(type),
           (long long)k, (long long)m, (long long)k, (long long)n, ms,
           (double)k * m * n * 2 / (ms * 1e-3) / 1e12);
    return 0;
}
